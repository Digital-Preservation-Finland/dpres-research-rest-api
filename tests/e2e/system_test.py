...
def test_tpas_preservation():
    response = post('http://metax-e2e-test.csc.fi/rest/v1/reset')
    assert response.status_code == 200
    response = get('http://admin-rest-api-e2e-test.csc.fi/api/1.0/datasets/'
                   '100')
    assert response.status_code == 200
    assert response.json()['passtate'] == 0
    response = post('http://admin-rest-api-e2e-test.csc.fi/api/1.0/datasets/'
                    '100/propose',
    ...
    response = post('http://research-rest-api-e2e-test.csc.fi/dataset/100/'
                    'genmetadata')
    assert response.status_code == 200
    response = get('http://admin-rest-api-e2e-test.csc.fi/api/1.0/datasets/'
                   '100')
    ...
    counter = 0
    passtate = 80
    while counter < 60 and passtate != 120:
        response = get('http://admin-rest-api-e2e-test.csc.fi/api/1.0/'
                       'datasets/100')
        assert response.status_code == 200
